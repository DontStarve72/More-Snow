...
public class MSBlocks {
	...
	public static void init(int minimumID) {
		snowyPlant = (BlockSnowyPlant) new BlockBuilder(MoreSnow.MOD_ID)
				...
				.build(new BlockSnowyPlant("snowy.plant", minimumID++, Material.topSnow, Block.tallgrass.id,
						Block.mushroomRed.id, new int[] { Block.algae.id }, false, false));
		...
		snowySlab = (BlockSnowySlab) new BlockBuilder(MoreSnow.MOD_ID)
				...
				.build(new BlockSnowySlab("snowy.slab", minimumID++, Material.snow, Block.slabPlanksOak.id,
						Block.slabBasaltPolished.id, new int[] { Block.slabPlanksOakPainted.id }, true, false));
		...
		snowySlabPainted = (BlockSnowySlabPainted) new BlockBuilder(MoreSnow.MOD_ID)
				...
				.build(new BlockSnowySlabPainted("snowy.slab.painted", minimumID++, Material.snow,
						Block.slabPlanksOakPainted.id, Block.slabPlanksOakPainted.id, new int[0], true, false));
		...
		snowyStairs = (BlockSnowyStairs) new BlockBuilder(MoreSnow.MOD_ID)
				...
				.build(new BlockSnowyStairs("snowy.stairs", minimumID++, Material.snow, Block.stairsPlanksOak.id,
						Block.stairsBrickStone.id, new int[] { Block.stairsPlanksOakPainted.id }, true, true));
		...
		snowyStairsPainted = (BlockSnowyStairsPainted) new BlockBuilder(MoreSnow.MOD_ID)
				...
				.setTags(BlockTags.MINEABLE_BY_SHOVEL)
				.setBlockModel(block -> new BlockModelSnowyStairs(block))
				.build(new BlockSnowyStairsPainted("snowy.stairs", minimumID++, Material.snow,
						Block.stairsPlanksOakPainted.id, Block.stairsPlanksOakPainted.id, new int[0], true, true));
